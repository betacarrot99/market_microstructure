...
def ewma_strategy_with_tuning(data, short_range, long_range):
    ...
    for short_window in short_range:
        for long_window in long_range:
            ...
            df = data.copy()
            df['shorter_EWMA'] = df['last_trade_price'].ewm(span=short_window, adjust = False).mean()
            df['longer_EWMA'] = df['last_trade_price'].ewm(span=long_window, adjust = False).mean()
            df['Signal'] = np.where(df['shorter_EWMA'] > df['longer_EWMA'], 1, 0)
...
best_params, train_results = ewma_strategy_with_tuning(
    train_data,
