...
sma_rows = span_df[span_df['method'] == 'SMA']
latest_sma = sma_rows.iloc[-1]  # Use the latest SMA config

CSV_FILE = 'result/live_data.csv'
SHORT_SPAN = int(latest_sma['short_window'])
LONG_SPAN = int(latest_sma['long_window'])
print(SHORT_SPAN, LONG_SPAN)
...
def calculate_signal(df):
    df['short_sma'] = df['price'].rolling(window=SHORT_SPAN).mean()
    df['long_sma'] = df['price'].rolling(window=LONG_SPAN).mean()
